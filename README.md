...
- Chromascope is the primary viewer. It draws its own input in Pump's fixed
  ...
  clicks activate sources for analysis; Command-click independently highlights
  up to eight active sources. Each highlight keeps a stable, distinct
  near-white tint mirrored in the source row and scope trace, and a full set
  refuses new entries rather than replacing an existing highlight.
