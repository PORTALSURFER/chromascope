...
x86_64-pc-windows-msvc VST3 target. The workflow never signs the binary, uses
no certificate, uploads no catalog entry, and does not create or attach a
GitHub Release.
...
revisions. The helper writes canonical JSON and uses fixed ZIP timestamps so
the same inputs produce the same archive and manifest.
