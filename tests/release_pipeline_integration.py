...
TEST_RELEASE_TOKEN = "chromascope-integration-release-token"
TEST_OIDC_REQUEST_TOKEN = "chromascope-integration-oidc-request-token"
TEST_ATTESTATION_TOKEN = "chromascope-integration-attestation-token"
TEST_NOTARY_ID = "00000000-0000-4000-8000-000000000000"
CAPABILITY_PATH = "/plugins/api/v1/products/chromascope/releases"
...
def _require_shared_source(manifest: dict[str, Any], source_sha: str, label: str) -> None:
    ...
    require(source.get("repository") == "PORTALSURFER/chromascope", f"{label} repository is invalid")
...
def _load_macos_preflight(
    ...
    *,
    ...
    require(manifest.get("product") == "chromascope", "macOS preflight product is invalid")
    ...
    require(isinstance(artifacts, list) and len(artifacts) == 1, "macOS preflight must contain exactly one archive")
    ...
    archive_name = artifacts[0].get("name")
    screenshot_name = screenshot.get("name")
    changelog_name = changelog.get("name")
    require(isinstance(archive_name, str), "macOS preflight archive metadata is invalid")
    ...
    require(archive_name == f"chromascope-v{package_version}-macos.vst3.zip", "macOS preflight archive name is invalid")
    require(len(list(root.glob("*-macos.vst3.zip"))) == 1, "macOS preflight must have exactly one macOS archive")
    require(len(list(root.glob("chromascope-default-*.png"))) == 1, "macOS preflight must have exactly one screenshot")
    require(len(list(root.glob("CHANGELOG.md"))) == 1, "macOS preflight must have exactly one changelog")
    _require_exact_entries(
        root,
        {"release-manifest.json", archive_name, screenshot_name, changelog_name},
...
def _load_windows_artifact(
    ...
    *,
    ...
    require(manifest.get("product") == "chromascope", "Windows artifact product is invalid")
...
class ApiMock:
    ...
    def _release_upload_prefix(self) -> str:
        ...
        return f"/plugins/api/v1/products/chromascope/release-uploads/{build_id}"
...
def _assembled_release(args: argparse.Namespace) -> Iterator[tuple[dict[str, Any], Path]]:
    ...
    with tempfile.TemporaryDirectory(prefix="chromascope-release-integration-", dir=scratch_parent) as directory:
        ...
        mac_archive_destination = scratch / f"chromascope-v{args.publication_version}-macos.vst3.zip"
        ...
        _copy_referenced(mac_archive, mac_archive_destination, "macOS preflight archive")
